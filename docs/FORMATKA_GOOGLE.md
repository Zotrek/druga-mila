...
Docelowy arkusz online (Apps Script dopisuje tu wiersze): [lista-druga-mila](https://docs.google.com/spreadsheets/d/1-qRyFnpjvAI1pZYkVXOUKKV9oYlxGsLidDXCtxYWzS0/edit?usp=sharing) — ID `1-qRyFnpjvAI1pZYkVXOUKKV9oYlxGsLidDXCtxYWzS0`. Wiersze trafiają na **zakładkę miesiąca** z daty odbioru (np. `Sierpień 2026`); przy pierwszym transporcie miesiąca zakładka jest tworzona z nagłówkami. Numeracja **DM*** ciągła przez zakładki (z pominięciem `GMH*`). Nagłówki online **zweryfikowane** — zgodne z mapowaniem poniżej (**16** kolumn).
...
**Stały odbiór (Harmonogram):** lista z zakładki `Harmonogram` → popup z proponowanymi datami z „Dzień odbioru” → N× `commitHarm` (seria **GMH***) + Word; wiersz Harmonogramu zostaje.
...
| 4 | Nr zlecenia transportowego | Ten sam numer co w Word (**auto** z API: `DM*` lub `GMH*` dla stałego odbioru) |
...
| 16 | Uwagi | Z pola „Uwagi” w modalu (**tylko Google** — nie Bolęcin, nie Word) |
