...
export function wordModalCss(): string {
  return `
    ...
    .doc-modal-panel input[type="text"], .doc-modal-panel input[type="search"], .doc-modal-panel input[type="date"], .doc-modal-panel input[type="number"], .doc-modal-panel select { width: 100%; padding: 8px 10px; font-size: 14px; border: 1px solid #ccc; border-radius: 6px; box-sizing: border-box; }
    ...
    .doc-date-range-field { display: flex; flex-direction: column; gap: 2px; min-width: 0; flex: 1 1 0; }
    .doc-date-range-field > span { font-size: 11px; font-weight: 600; color: #555; }
    .doc-date-range-field .doc-date-row { width: 100%; }
    .doc-date-range-field input[type="text"] { flex: 1; min-width: 0; width: auto; padding: 6px 8px; font-size: 13px; }
    .doc-date-range-hint { font-size: 11px; color: #666; margin: 4px 0 0; width: 100%; flex: 1 0 100%; }
    ...
    .map-manual-gen-wrap { margin-top: 8px; padding-top: 8px; border-top: 1px solid #e8e8e8; display: flex; flex-direction: column; gap: 4px; }
    .map-manual-generate { width: 100%; padding: 6px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #0d6efd; background: #0d6efd; color: #fff; cursor: pointer; }
    .map-manual-generate:hover { filter: brightness(1.05); }
    .map-manual-bulk-generate { width: 100%; padding: 6px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #198754; background: #198754; color: #fff; cursor: pointer; }
    .map-manual-bulk-generate:hover { filter: brightness(1.05); }
    .map-manual-combined-generate { width: 100%; padding: 6px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #b45309; background: #b45309; color: #fff; cursor: pointer; }
    .map-manual-combined-generate:hover { filter: brightness(1.05); }
    .map-planowane-generate { width: 100%; padding: 6px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #6f42c1; background: #6f42c1; color: #fff; cursor: pointer; }
    .map-planowane-generate:hover { filter: brightness(1.05); }
    .map-harmonogram-generate { width: 100%; padding: 6px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #0d9488; background: #0d9488; color: #fff; cursor: pointer; }
    ...
    #planowane-picker .harm-list-row:hover { border-color: #6f42c1; background: #f3eef9; }
    ...
    .harm-list-actions { display: flex; flex-wrap: nowrap; gap: 6px; justify-content: flex-end; }
    .harm-icon-btn { display: inline-flex; align-items: center; justify-content: center; width: 34px; height: 34px; padding: 5px; border-radius: 6px; border: 1px solid #ccc; background: #f8f8f8; cursor: pointer; flex: 0 0 auto; line-height: 0; }
    .harm-icon-btn img { width: 18px; height: 18px; display: block; pointer-events: none; }
    .harm-icon-btn.harm-edit { border-color: #0d9488; background: #f0fdfa; }
    .harm-icon-btn.danger { border-color: #dc3545; background: #fff5f5; }
    .harm-icon-btn:hover { filter: brightness(0.97); }
    .harm-icon-btn:disabled { opacity: 0.5; cursor: not-allowed; }
    .doc-modal-actions .harm-icon-btn { width: 36px; height: 36px; }
    ...
    .map-bulk-panel { margin-top: 8px; padding-top: 8px; border-top: 1px solid #e8e8e8; display: flex; flex-wrap: wrap; align-items: center; gap: 6px; }
    .map-bulk-panel[hidden] { display: none !important; }
    .map-bulk-count { font-size: 11px; color: #333; flex: 1; min-width: 100px; }
    .map-bulk-generate { padding: 5px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #198754; background: #198754; color: #fff; cursor: pointer; }
    .map-bulk-clear { padding: 5px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #ccc; background: #f8f9fa; cursor: pointer; }
    .map-combined-generate { padding: 5px 8px; font-size: 11px; border-radius: 5px; border: 1px solid #b45309; background: #b45309; color: #fff; cursor: pointer; }
...
export function wordModalHtml(): string {
  ...
  <div id="harmonogram-picker" class="doc-modal-overlay" style="display:none" aria-hidden="true">
    <div class="doc-modal-panel" role="dialog" aria-labelledby="harmonogram-title">
      ...
      <label for="harmonogram-search">Szukaj</label>
      <input type="search" id="harmonogram-search" placeholder="Nazwa / adres / dzień / kto odbiera…" autocomplete="off" spellcheck="false" />
  ...
  <div id="harmonogram-add" class="doc-modal-overlay" style="display:none" aria-hidden="true">
    <div class="doc-modal-panel" role="dialog" aria-labelledby="harmonogram-add-title">
      ...
      <div class="doc-modal-actions">
        ...
        <button type="button" id="harmonogram-add-delete" class="harm-icon-btn danger" hidden title="Usuń" aria-label="Usuń z Harmonogramu"><img src="./icon-delete.png" alt="" width="18" height="18" /></button>
...
export function wordModalBrowserScript(): string {
  return `
        ...
        .then(function(data) {
          ...
          if (statusEl) {
            statusEl.textContent = rows.length === 0
              ...
              : (rows.length + ' planowanych — Realizuj / Edytuj / Usuń.');
          }
          rows.forEach(function(row) {
            ...
            mainBtn.title = 'Realizuj planowane';
            ...
            editBtn.type = 'button';
            editBtn.className = 'harm-icon-btn harm-edit';
            editBtn.title = 'Edytuj';
            editBtn.setAttribute('aria-label', 'Edytuj');
            editBtn.innerHTML = '<img src="./icon-edit.png" alt="" width="18" height="18" />';
            ...
            delBtn.type = 'button';
            delBtn.className = 'harm-icon-btn danger';
            delBtn.title = 'Usuń';
            delBtn.setAttribute('aria-label', 'Usuń');
            delBtn.innerHTML = '<img src="./icon-delete.png" alt="" width="18" height="18" />';
            delBtn.addEventListener('click', function(ev) {
              ev.stopPropagation();
              deletePlanowaneRow(row);
            });
    ...
    function openHarmonogramPicker() {
      ...
      if (!m || !wordDocEnabled) return;
      var searchEl = document.getElementById('harmonogram-search');
      if (searchEl) searchEl.value = '';
      m.style.display = 'flex';
      m.setAttribute('aria-hidden', 'false');
      loadHarmonogramList();
    ...
    function harmonogramRowHaystack(row) {
      return normQ([
        row.nazwaKontrahenta,
        row.nazwaKontrahentaIi,
        row.adresOdbioru,
        row.adresOdbioruIi,
        row.dzienOdbioru,
        row.czestotliwosc,
        ...
        row.uwagi,
        row.stawka,
        row.rodzajTransportu,
        row.awizacja,
        row.znacznikMiejsca,
      ].filter(Boolean).join(' '));
    }
    function renderHarmonogramList() {
      if (!window.__harmonogramLoaded) return;
      var statusEl = document.getElementById('harmonogram-status');
      var listEl = document.getElementById('harmonogram-list');
      var searchEl = document.getElementById('harmonogram-search');
      if (!listEl) return;
      var q = normQ(searchEl ? searchEl.value : '');
      var rows = window.__harmonogramRows || [];
      var filtered = [];
      for (var i = 0; i < rows.length; i++) {
        if (!q || harmonogramRowHaystack(rows[i]).indexOf(q) >= 0) {
      ...
      if (statusEl) {
        if (rows.length === 0) {
          statusEl.textContent = 'Brak stałych odbiorów w Harmonogramie.';
        } else if (filtered.length === 0) {
          statusEl.textContent = 'Brak wyników.';
        } else if (q) {
          statusEl.textContent = filtered.length + ' z ' + rows.length + ' — Generuj / Edytuj / Usuń.';
        } else {
          statusEl.textContent = rows.length + ' stałych — Generuj / Edytuj / Usuń.';
        }
      }
    ...
    function loadHarmonogramList() {
      ...
      fetch(WEBAPP_URL + (WEBAPP_URL.indexOf('?') >= 0 ? '&' : '?') + 'action=listHarmonogram')
        ...
        .then(function(data) {
          ...
          window.__harmonogramRows = data.rows || [];
          window.__harmonogramLoaded = true;
          renderHarmonogramList();
        })
        .catch(function(err) {
          if (seq !== harmonogramListSeq) return;
          window.__harmonogramLoaded = false;
    ...
    function deletePlanowaneRow(row) {
      if (!WEBAPP_URL) {
        alert('Usuwanie wymaga Web App.');
        return;
      }
      ...
      if (!window.confirm('Usunąć ' + (row.numer || '') + ' z Planowane? Numer wróci do puli.')) {
        return;
      }
      ...
      appendFormatkaRow({ mode: 'deletePlan', planowaneRow: row.rowIndex }).then(function(resp) {
        ...
        if (window.__realizePlan && window.__realizePlan.rowIndex === row.rowIndex) {
          closeDocModal();
        }
        openPlanowanePicker();
      }).catch(function(err) {
    ...
    var harmonogramSearch = document.getElementById('harmonogram-search');
    ...
    if (harmonogramSearch) harmonogramSearch.addEventListener('input', renderHarmonogramList);
